  ...
  */
  ...
  Aura.prototype.createSandbox = function() {
    ...
    if(this.config.debug){
      //Create new instance of Logger for each sandbox
      sandbox.logger = new Logger();
  ...
  Aura.prototype.start = function(options) {
    ...
    if (app.started) {
      app.sandbox.logger.error("Aura already started... !");
      return app.extensions.initStatus;
    }
    app.sandbox.logger.log("Starting Aura...");
      ...
      base.util.each(exts, function(i, ext) {
        if (ext && typeof(ext.afterAppStart) === 'function') {
          try {
            ...
            app.sandbox.logger.error("Error on ", (ext.name || ext) , ".afterAppStart callback: (", e.message, ")", e);
          }
        }
      });
    });
    ...
    app.extensions.onFailure(function() {
      app.sandbox.logger.error("Error initializing app...", app.config.name, arguments);
      app.stop();
    });
