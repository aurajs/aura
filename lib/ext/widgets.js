define('aura/ext/widgets', function() {
  ...
  return function(app) {
    ...
    function Widget(options) {
      ...
      this.$el[0].addEventListener("DOMNodeRemoved", function() {
        console.warn("Stopping sadnbox !", this);
        this.sandbox.stop();
      }, false);
    ...
    Widget.load = function(name, options) {
      ...
      // it's too big !
      // 
      console.warn("Loading widget ", name);
    ...
    return {
      ...
      initialize: function(app) {
        ...
        app.sandbox.start = function() {
          app.core.mediator.emit(['aura', 'sandbox', 'start'].join('.'), this);
          app.core.start.apply(app.core, arguments);
        };

        app.sandbox.stop = function() {
          app.core.mediator.emit(['aura', 'sandbox', 'stop'].join('.'), this);
        };
