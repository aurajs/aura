define('aura/ext/widgets', function() {
  ...
  return function(app) {
    ...
    var widgetsCallbacks = {};


    function invokeCallbacks(stage, fnName, context, args) {
      var callbacks = widgetsCallbacks[stage + ":" + fnName] || [];
      var dfds      = [];
    ...
    app.core.util.invokeWithCallbacks = invokeWithCallbacks;
     ...
     */
    Widget.load = function(name, opts) {
      ...
      require([ref], function(widgetDefinition) {
        ...
        try {
          ...
          newWidget.$el.data('__sandbox_ref__', sandbox.ref);
     ...
     */
    function parseWidgetOptions(el, namespace, options) {

      var ret = _.clone(options || {});
      ret.el = el;
      ret.require = {};
      ...
      return buildWidgetOptions(name, ret);
    }

    function buildWidgetOptions(name, options) {
      var ref          = name.split("@"),
    ...
    Widget.parseList = function(widgets) {
      ...
      if (Array.isArray(widgets)) {
        _.map(widgets, function(w) {
          var options = buildWidgetOptions(w.name, w.options);
          list.push({ name: w.name, options: options });
        });
      } else if (widgets && core.dom.find(widgets)) {
    ...
    Widget.startAll = function(widgets) {
      ...
      core.util.each(widgetsList, function(i, w) {
        var ret = Widget.load(w.name, w.options);
        list.push(ret);
      });
      var loadedWidgets = core.data.when.apply(undefined, list);
      return loadedWidgets.promise();
    };
    ...
    return {
      ...
      initialize: function(app) {
        ...
        app.core.registerWidgetCallback = function(callbackName, fn) {
          widgetsCallbacks[callbackName] = widgetsCallbacks[callbackName] || [];
          widgetsCallbacks[callbackName].push(fn);
        };
        ...
        app.sandbox.start = function (list, options) {
          ...
          Widget.startAll(list).done(function () {
        ...
        function stopSandbox(sandbox) {
          ...
          if (sandbox) {
            ...
            if (sandbox._widget) {
              invokeWithCallbacks('remove', sandbox._widget);
            }
        ...
        app.sandbox.stop = function(selector) {
          if (selector) {
            var $el = core.dom.find(this.el);
            core.dom.find(selector, $el).each(function(i, el) {
      ...
      afterAppStart: function(app) {
        ...
        if (app.startOptions.widgets) {
          var el;
          if (Array.isArray(app.startOptions.widgets)) {
            el = core.dom.find('body');
          } else {
            el = core.dom.find(app.startOptions.widgets);
          }
          app.core.appSandbox = app.createSandbox(app.ref, { el: el });
          app.core.appSandbox.start(app.startOptions.widgets);
        }
      }
