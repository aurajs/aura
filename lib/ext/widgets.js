define('aura/ext/widgets', function() {
  ...
  return function(app) {
    ...
    Widget.load = function(name, options) {
      ...
      // it's too big !
      //
      app.sandbox.logger.log("Start loading widget:", name);
      ...
      dfd.fail(function(err) {
        app.sandbox.logger.error("Error loading widget:", name, err);
      });
      ...
      require([options.ref], function(widgetDefinition) {
        ...
        try {
          ...
          sandbox.logger.setName('Widget: ' + name);
          ...
          app.sandbox.logger.error(err.message);
          dfd.reject(err);
        }
